...
// Function to load tokenized data from binary file
pub fn load_tokenized_data(filename: &str) -> TokenizedData {
    let mut file = File::open(filename).expect("Failed to open file");
    let mut buffer = Vec::new();
    file.read_to_end(&mut buffer).expect("Failed to read file");
    
    bincode::deserialize(&buffer).expect("Failed to deserialize tokenized data")
}
