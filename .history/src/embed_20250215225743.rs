...
pub fn embed_chunks(chunks: Vec<String>, model_path: &str) -> Vec<EmbeddedChunk> {
    ...
    for chunk in chunks {
        ...
        // Run the model
        let outputs: Vec<OrtOwnedTensor<f32>> = session.run(vec![input_tensor])
            .expect("Model inference failed");

        // Extract the output vector
        let vector = outputs[0].to_vec();
...
fn main() {
    ...
    save_embeddings("embeddings.bin", &embeddings);

    let loaded_embeddings = load_embeddings("embeddings.bin");
    println!("Loaded embeddings: {:?}", loaded_embeddings);
}